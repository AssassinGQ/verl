...
for CONCURRENCY in "${concurrencys[@]}"; do
    for CONTEXT in "${contexts[@]}"; do
        ...
        sed -i "s|    max_turns: .*|    max_turns: ${MAX_TURNS}|" $DEPLOYMENT
        ...
        while ! grep -q "$TARGET" "$LOG_FILE" 2>/dev/null; do
            pkill -9 -f 'parallel_infer.py|ray::'
        ...
        for lt in "${lts[@]}"; do
            ...
            sed -i "s|  log_dir: .*|  log_dir: ${LOG_BASE}/router-trajs/kvcaware-lt${lt}-${CONCURRENCY}-${CONTEXT}|" $DEPLOYMENT
            while ! grep -q "$TARGET" "$LOG_FILE" 2>/dev/null; do
                pkill -9 -f 'parallel_infer.py|ray::'
                ps -aux | grep run_infer.sh | grep -v grep | awk -F ' ' '{print$2}' | xargs -I {} kill -9 {}
                ray stop
