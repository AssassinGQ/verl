...
#
# 6 nodes (this host = Ray head, + 5 ssh workers, each entered via
...
step3_matrix() {
    ...
    for CONCURRENCY in "${concurrencys[@]}"; do
        for CONTEXT in "${contexts[@]}"; do
            ...
            export SWE_AGENT_DUMP_TRAJECTORIES="infer-sticky-prompt${MAX_SAMPLES}x8-${CONCURRENCY}x${CONTEXT}-n${NNODES}.traj"
            ...
            for lt in "${lts[@]}"; do
                ...
                export SWE_AGENT_DUMP_TRAJECTORIES="infer-kvcaware-lt${lt}-prompt${MAX_SAMPLES}x8-${CONCURRENCY}x${CONTEXT}-n${NNODES}.log"
