...
Two mount points feed the same singleton (see planning findings §19-20):
...
class WriteEvent:
    ...
    Attributes:
        ...
        request_id: The routed request id (acquire/release); ``None`` otherwise.
...
class Emitter:
    ...
    def on_write(self, event: WriteEvent) -> None:
        ...
        e.g. inflight_tokens, fire from more than one kind). See planning
        findings §23/§24 for the per-kind mapping.
        """
        if not self.enabled():
            return
        self._ensure_init()
        ...
        if kind == WriteKind.POLL:
            for key in _POLL_GAUGES:
                self._emit_gauge_abs(key, event, replica)
            if event.load is not None:
                self._emit(EmitKey.KV_CACHE_LOAD, event.load, replica)
        elif kind == WriteKind.ACQUIRE:
            self._emit_counter_delta(EmitKey.DISPATCHED_COUNT, event, replica)
            self._emit_counter_delta(EmitKey.PROMPT_LEN_SUM, event, replica)
            self._emit_gauge_abs(EmitKey.INFLIGHT_TOKENS, event, replica)
            self._emit_avg_turn(replica, event)
        elif kind == WriteKind.RELEASE:
            self._emit_counter_delta(EmitKey.COMPLETED_COUNT, event, replica)
            self._emit_gauge_abs(EmitKey.INFLIGHT_TOKENS, event, replica)
            self._emit_avg_turn(replica, event)
        elif kind == WriteKind.KV_REMOVED:
            self._emit_counter_delta(EmitKey.KV_EVICTIONS, event, replica)

    def _emit_counter_delta(self, key: str, event: WriteEvent, replica: str) -> None:
        delta = event.deltas.get(key)
        if delta is not None:
            self._emit(key, delta, replica)

    def _emit_gauge_abs(self, key: str, event: WriteEvent, replica: str) -> None:
        value = event.new_values.get(key)
    ...
    def _emit_avg_turn(self, replica: str, event: WriteEvent) -> None:
        ...
        Per §24: the store/collector supplies the two absolutes; the emitter
