...
class LLMServerClient:
    ...
    def _release_server(self, server_id: str, prompt_len: int = 0, request_id: str | None = None) -> None:
        ...
        # request_id lets the kvc-aware router attribute the release to the right request
        # (e.g. subtract its turn from the in-flight turn sum); ignored by balancers that
        # track request counts only.
        self._load_balancer.release_server.remote(server_id=server_id, prompt_len=prompt_len, request_id=request_id)
...
class FullyAsyncLLMServerClient(LLMServerClient):
    ...
    async def _acquire_server(self, request_id: str, prompt_ids: list[int] = None) -> tuple[str, ray.actor.ActorHandle]:
        ...
        while True:
            try:
                return await super()._acquire_server(request_id, prompt_ids)
...
class LLMServerManager:
    ...
    Args:
        ...
        load_balancer_cls: Optional subclass of the default router strategy's
            ...
            over the ``rollout.router`` config section. Pass a subclass that
    ...
    def __init__(
        ...
        # Store as-is; router.get_router_handle() resolves the default class
        # (None → router.GlobalRequestLoadBalancer) at LB init time.
    ...
    async def _init_global_load_balancer(self) -> None:
        ...
        self.global_load_balancer = get_router_handle(
            ...
            router_config_path=self.rollout_config.router_config_path,
